import unittest
from skmultilearn.model_selection.tests.fixtures import FIXTURE_Y, FOLDS_FIXTURES
from skmultilearn.model_selection import (
    example_distribution, label_combination_distribution, percentage_of_label_combinations_without_evidence_per_fold,
    folds_without_evidence_for_at_least_one_label_combination, folds_label_combination_pairs_without_evidence
)
...
class FoldMeasuresTests(unittest.TestCase):
    ...
    def test_folds_combination_pairs_without_evidence_counts_proper_values(self):
        for fold_data in FOLDS_FIXTURES:
            ...
            self.assertEqual(
                folds_label_combination_pairs_without_evidence(FIXTURE_Y, folds, order=1),
                expected_fold_count
            )
            self.assertEqual(
